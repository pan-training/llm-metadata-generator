...
_SMALL_TASKS = {"link_decision", "model_selection"}
_EMBEDDING_TASKS = {"ontology_embedding"}
# All other tasks (content_relevance, content_summary, json_ld_review,
# metadata_analysis, tool_discovery, …) use the large model.
...
def get_model_for_task(task: str = "default") -> str:
    ...
    Task tiers:
      small  – LLM_MODEL_SMALL (default: qwen2.5-coder-7b-instruct)
               Fast, cheap: link_decision, model_selection.
      large  – LLM_MODEL_LARGE (default: gemma-3-27b-it)
               Quality: content_relevance, content_summary, json_ld_review,
               metadata_analysis, tool_discovery, default.
